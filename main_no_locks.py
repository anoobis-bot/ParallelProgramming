...
class Worker(threading.Thread):
    def __init__(self, images_to_process, model, debug_folder, threadID):
    ...
    def process_image(self, image_path, model, debug_folder, results):
        ...
        results.append((os.path.basename(image_path), plate_text))
...
def get_image_files(image_directory):
    ...
    return [os.path.join(image_directory, f) for f in os.listdir(image_directory) if
            f.lower().endswith(('png', 'jpg', 'jpeg'))]


def process_chunk(chunk, image_directory):
    # Process a chunk of files
    return [os.path.join(image_directory, f) for f in chunk if f.lower().endswith(('png', 'jpg', 'jpeg'))]


def get_image_files_concurrently(image_directory, num_threads=4):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        result = list(executor.map(lambda chunk: process_chunk(chunk, image_directory), chunks))
...
def main():
    ...
    image_files = get_image_files_concurrently(image_directory, num_threads=4)
    ...
    for batch in image_batches:
        ...
        worker = Worker(batch, model, debug_folder, i)
    ...
    for worker in threads:
        worker.join()
        results.append(worker.get_results())
    ...
    with open(output_stats, 'a') as f:
        ...
        f.write(f"Remarks: Used domain decomposition with {num_workers} number of workers\n")
        f.write("--------------------------------\n\n")
    
    print(f"Processed {num_processed} images, identified {num_identified} plates in {total_time:.2f} seconds. Also used multiprogramming with loading images")
