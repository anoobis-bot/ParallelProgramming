...
def process_image(image_path, model, debug_folder, results):
...
def main():
    ...
    for image_path in tqdm(image_files, total=len(image_files), desc="Processed images"):
        if len(threads) >= num_workers:
            for thread in threads:
                thread.join()
            threads.clear()

        thread = threading.Thread(target=process_image, args=(image_path, model, debug_folder, results))
    ...
    with open(output_stats, 'a') as f:
        ...
        f.write(f"Remarks: Used for loops with join with {num_workers}\n")
        f.write("--------------------------------\n\n")
